...
const RecentComments = () => {
  ...
  return (
    <Card>
      <CardBody>
        <CardTitle tag="h4">Recommended</CardTitle>
      </CardBody>
      <SimpleBar style={{ height: '600px' }}>
        <ListGroup flush>
          {projects.map((project) => (
            <ListGroupItem action href="#" tag="a" key={project.id} className="d-flex p-3 w-100">
              <div>
                <img src={user1} alt="user" width="50" className="rounded-circle me-3" />
              </div>
              <div className="w-100">
                <ListGroupItemHeading className="fw-medium mb-0">
                  {project.name}
